...
- [3章 二分探索](#3章-二分探索)
  ...
  - [3.5 チャレンジ問題](#35-チャレンジ問題)
    - [A15 Compression ★3](#a15-compression-3)
- [4章 動的計画](#4章-動的計画)
...
- [6章 考察テクニック](#6章-考察テクニック)
  - [6.0 考察テクニック入門](#60-考察テクニック入門)
  - [6.1 偶奇を考える](#61-偶奇を考える)
    - [A36 Travel ★2](#a36-travel-2)
    - [B36 Switching Lights](#b36-switching-lights)
...
Aの数字の大小関係を崩さずに、値を最小化する問題。
本には色々書いてあるが、簡単な話やるのはこれだけ
1. **set** 等で重複無しのソート配列を作る
2. 値の小さい順にAの値をキーとした辞書型の変数に小さい順に1から順に割り当ててる
3. 辞書型の変数を使って、Aの配列の値をBの値になるように変換して出力
...
int main(){
    ...
    vector<ulong> B(N,0);
    set<ulong> S;
    for(ulong i = 0; i < N; i ++) {cin >> A[i]; S.insert(A[i]);}
...
# 6章 考察テクニック
## 6.0 考察テクニック入門
以下のような4×6のマス目に、6個のピースを敷き詰める事が可能かどうかを考えてみる。ピースは回転させては良いが、反転は出来ないモノとする
![](images/6_block.png)

プログラミングではなく、手で動かして敷き詰める分には簡単であり、28通りの敷き詰め方がある。
しかし、このような問題をプログラミングではどのように一般化して敷き詰められるかどうかの証明を行うにはどうしたらよいか

この問題の場合は、以下のように考える。
1. 偶数と奇数のマス目にそれぞれ白と黒の色を塗って区別する
2. 各ブロックを置いた時に、各ブロックに偶数と奇数のマス目がそれぞれ何個含まれるかを数える。
上の例の場合、凸型のブロック以外はどこに置いても白黒のブロックが二つずつ含まれ、凸型のブロックでは白黒のブロックどちらかが3個含まれ、どちらかが1個含まれる形となる
3. マス目は24個で白と黒のブロックはそれぞれ12個ずつのため、ブロックに含まれる白と黒のブロックの個数のパターンを考え、12個ずつ含まれるパターンがある場合は置くことが可能であると考える事ができ、12個ずつ置けない場合はどうしても、1つ余ってしまうため、置くことが出来ないと判断することが可能

このような考察力やひらめきが要求される場面は競技プログラミングにおいて少なくない。 
この章ではこういった「典型考察パターン10選」を学び、考察が苦手な人でも理解できるようにする

## 6.1 偶奇を考える
### A36 Travel ★2
問題文:https://atcoder.jp/contests/tessoku-book/tasks/tessoku_book_aj

左上から右下までに辿りつくための、最短経路は $(N - 1) * 2$ なので、Kがそれ以下の場合はその時点で辿り付けない事が分かる。 
(この判定時に非負型を使わないようにしましょう(二敗))


たどり付ける場合、残りの手順で隣のマスと往復し続けることを考えると、通ってきた最短経路分を引いた残りの手順数が **偶数** になっていれば、往復して右下に丁度辿り着くことができるが、 **奇数** の場合はピッタリ辿りつくことが出来ないのでそれで判定
...
問題文:https://atcoder.jp/contests/tessoku-book/tasks/tessoku_book_di
...
- 動的計画法
  - [動的計画法](#41-動的計画法の基本)
  - [部分和問題](#43-二次元のdp1部分和問題)
  - [ナップザック問題](#44-二次元のdp2ナップザック問題)
  - [最長共通部分列問題](#45-二次元のdp3最長共通部分列問題)
- グラフ
  - [深さ優先探索](#92-深さ優先探索)
  - [幅優先探索](#93-幅優先探索)
  - [ダイクストラ法](#94-ダイクストラ法)
  - [Union Find](#96-union-find-木)
  - [最小全域木](#97-最小全域木問題)